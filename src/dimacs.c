#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
#include <string.h>

#include "primitives.h"


static void ignore_comment_lines(FILE* STREAM) {
    assert(STREAM);
    char line[1024];

    do {
        char* error1 = fgets(line, sizeof(line), STREAM);
        assert(error1);
    } while(line[0] == 'c');
    
    i32 error2 = fseek(STREAM, -strlen(line), SEEK_CUR);

    assert(!error2);
    assert(line[0] == 'p');
}

static void scan_order_and_size(FILE* STREAM, i32* ptr_nvertices, i32* ptr_nedges) {
    assert(STREAM);
    assert(ptr_nvertices);
    assert(ptr_nedges);

    const char TEMPLATE[] = "p edge " i32_SCN " " i32_SCN;
    i32 nassignments = fscanf(STREAM, TEMPLATE, ptr_nvertices, ptr_nedges);

    assert(nassignments == 2);
}

static void scan_edges(FILE* STREAM, u32* edges, i32 nedges) {
    assert(STREAM);
    assert(edges);
    assert(nedges > 0);

    for(i32 i = 0; i < nedges; i++) {
        i32 nassignments;
        u32 name1, name2;
        const char TEMPLATE[] = "\ne " u32_SCN " " u32_SCN;

        nassignments = fscanf(STREAM, TEMPLATE, &name1, &name2);
        assert(nassignments == 2);

        edges[2*i]     = name1;
        edges[2*i + 1] = name2;
    }
}

u32* dimacs_scan(FILE* STREAM, i32* ptr_nvertices, i32* ptr_nedges) {
    assert(STREAM);
    assert(ptr_nvertices);
    assert(ptr_nedges);

    i32 nvertices, nedges;
    ignore_comment_lines(STREAM);
    scan_order_and_size(STREAM, &nvertices, &nedges);

    u32* edges = malloc(2*nedges*sizeof(u32));
    scan_edges(STREAM, edges, nedges);

    *ptr_nvertices = nvertices;
    *ptr_nedges = nedges;

    assert(edges);
    assert(nvertices > 0);
    assert(nedges > 0);

    return edges;
}

u32* dimacs_print(FILE* STREAM, i32 nvertices, i32 nedges, u32* edges) {
    assert(STREAM);
    assert(nvertices > 0);
    assert(nedges > 0);
    assert(edges);

    fprintf(STREAM, "p edge " i32_PRI " " i32_PRI "\n", nvertices, nedges);
    for(i32 i = 0; i < nedges; i++)
        fprintf(STREAM, "e " u32_PRI " " u32_PRI "\n", edges[2*i], edges[2*i + 1]);

    return edges;
}